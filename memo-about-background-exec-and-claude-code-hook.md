...
### /bin/shでバックグラウンド実行したとき、エンターキーの入力を待つ + stdout, stderrは親プロセス側と共有する
エンターを押すと再度入力できるように
```
...
### stdout, steerrを/dev/nullにリダイレクトする+バックグラウンド実行すると待たずに終了する
```
...
- shの実装として、バックグラウンドプロセスのstdout, stderrを待つ
    - プロセスが終了するのか、
    - stdout, stderrに何が接続されているのかを管理しており、子プロセスが